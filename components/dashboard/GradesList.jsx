...
export default function GradesList(user) {
  ...
  function GradeBox({ studentGrade, gradeName, date, link }) {
    return (
      <div className="flex items-center justify-between gap-[30px] py-[8px] px-[30px] bg-[#F4F4F4] rounded-[10px] hover:scale-[.98] cursor-pointer transition-all duration-300 ease-in-out">

        <Box sx={{ position: 'relative', display: 'inline-flex' }}>

          <CircularProgress
            size={45} variant="determinate"
            sx={({ color: `${user.userColor}`, [`& .${circularProgressClasses.circle}`]: { strokeLinecap: 'round', } })}
        ...
        <p className='w-[100%] whitespace-nowrap overflow-hidden text-ellipsis'>{gradeName}</p>

        <div className='flex gap-[30px] items-center'>
          <p className='whitespace-nowrap'>{date}</p>
          <a className='hover:scale-[.9] transition-all duration-300 ease-in-out' href={link} target='_blank'><SquareArrowOutUpRight color={user.userColor} size={18} /></a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={circleRef} className='h-full overflow-hidden flex flex-col justify-between'>
      ...
      <div className='flex flex-col gap-[7px] h-[60%] overflow-y-scroll overflow-x-hidden scrollbar-custom pr-[5px]' id={user.userColor}>
        {response.length > 0 ? (
